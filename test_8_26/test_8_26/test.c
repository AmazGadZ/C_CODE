...
//	printf("%d\n", sizeof(a[0] + 1));//4/8 - a[0]是第一行的数组名，数组名此时是首元素的地址，
//	//a[0]其实就是第一行第一个元素的地址，所以a[0]+1 就是第一行第二个元素的地址 - 地址大小是4/8个字节
...
//	printf("%d\n", sizeof(a[3]));//16 - sizeof()内部表达式不真实计算，和sizeof(a[0])一样
//
//	//char* p = "abcdef";
//
//	//printf("%d\n", strlen(p));//6
